...
.controller('HistoryCtrl', function($scope, $log, $stateParams, $state, Tools, Lottery) {
	...
	if($scope.queryData.gameId == 70 || $scope.queryData.gameId== 113) {
...
.controller('LuZhuCtrl', function($scope, $log, $stateParams, Tools, Lottery, $state) {
	...
	for(var g in gameList){
		var game = gameList[g];
		if(game.id==10 || game.id==105 || game.id==114 || game.id==115 || game.id==116 || game.id==113 || game.id==70){//去除江苏快3 、 香港六合彩
			continue;
		}
	...
	$scope.queryData = {};
	$scope.queryData.gameId = parseInt($stateParams.gameId || Lottery.getFirstGameId());
	if($scope.queryData.gameId==10 || $scope.queryData.gameId==105 || $scope.queryData.gameId==114 || $scope.queryData.gameId==115 || $scope.queryData.gameId==116 || $scope.queryData.gameId==113 || $scope.queryData.gameId==70){
		Tools.tip('无此类游戏路珠');
