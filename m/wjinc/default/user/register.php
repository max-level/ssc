<?php
if (isset($_GET['link'])) {
    $linkData = $this->getRow("select * from {$this->prename}links where link=?", $_GET['link']);
}
?>
<!DOCTYPE html>
<html>
...
<form action="/index.php/user/registered" method="post" onajax="registerBeforSubmit" enter="true" call="registerSubmit"
      target="ajax">
    <div class="container cmargin20 cmargintop2 subcontent2">
        ...
        <?php
            if (isset($linkData) && !empty($linkData)) {
                ...
                <div class="rowfield clearfix">
                    <div class="col1 fl margintop15">推荐账号：</div>
                    <div class="col2 fl"><?=$linkData['uid']?>
                        <input type="hidden" id="parentId" name="parentId" class="text field_input" value="<?=$linkData['uid']?>">
                        <input type="hidden" id="lid" name="lid" class="text field_input" value="<?=$linkData['lid']?>">
                        <div class="red cnsmall1" id='nameTips'></div>
                    </div>
                </div>
                <?php
            } else {
        ?>
                <div class="rowfield clearfix">
                    <div class="col1 fl margintop15">推荐账号：</div>
                    <div class="col2 fl"><input type="text" id="parentId" name="parentId" class="text field_input">
                        <div class="red cnsmall1" id='nameTips'>无介绍人可不填写,但无反水,详情联系在线客服</div>
                    </div>
                </div>
        <?php } ?>
        ...
        <div class="rowfield clearfix">
            ...
            <div class="col2 fl"><input type="text" name="vcode" id="vcode" class="field_input2 fl">
                <div class="captcha capcha fl"><img width="178" height="64" alt="验证码" align="absmiddle"
                                                    src="/user/vcode/<?= $this->time ?>" title="看不清楚，点我换图"
                                                    onclick="this.src='/user/vcode/'+(new Date()).getTime()"/></div>
            </div>
        </div>
    </div>
    <div class="rowfield clearfix">
