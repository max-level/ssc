...
if ($this->user['testFlag'] == 1) {
    $querySql = "select * from {$this->prename}guestbets where  isDelete=0 and uid={$this->user['uid']} {$sql}";
    $list = $this->getPage($querySql, $pagenum, $pagesize);
    $listTotal = $this->getRows($querySql);
} else {
    $querySql = "select * from {$this->prename}bets where  isDelete=0 and uid={$this->user['uid']} {$sql}";
    $list = $this->getPage($querySql, $pagenum, $pagesize);
    $listTotal = $this->getRows($querySql);
}
...
if ($list['data']) {
    ...
    foreach ($listTotal as $key => $var) {
        ...
        if ($settled == 'true') {
            //已结算明细
            $resultMoney = floatval(sprintf("%.2f", $var['bonus'] - $tzmoney + $tzmoney * $var['rebate'])); //未结明细为可赢金额//已结明细为赢亏结果包括退水

        } else {
            //未结算明细
            $dataarr['status'] = 0;
            $dataarr['money'] = floatval($tzmoney);
            $resultMoney = floatval(sprintf("%.2f", $var['money'] * $var['odds'] - $tzmoney + $tzmoney * $var['rebate']));//未结明细可赢额金包括退水//已结明细结果包括退水

        }

        $totalResultMoney += $resultMoney;
        $totalBetMoney += $tzmoney;

    }
    $allarr['otherData']['totalResultMoney'] = $totalResultMoney;
    $allarr['otherData']['totalBetMoney'] = $totalBetMoney;
