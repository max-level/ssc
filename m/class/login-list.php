<?php
	...
	if($this->user['uid']==1){
	   $sql="select l.* from {$this->prename}sysadmin_session l, {$this->prename}sysmember u where u.uid=l.uid and admin=1 $timeWhere $userWhere order by loginTime desc";
	}else{
	   $sql="select l.* from {$this->prename}sysadmin_session l, {$this->prename}sysmember u where u.uid=l.uid and admin=1 $timeWhere $userWhere and uid={$this->user['uid']} order by loginTime desc";
	}
	//echo $sql;exit;
