		...
		res.on("end", function(){
			try{
				try{
					...
					if(data.type=='73' || data.type=='81'||data.type=='100' || data.type=='101' || data.type=='102' || data.type=='103' || data.type=='104' || data.type=='105'  || data.type=='106'|| data.type=='107'  || data.type=='109' || data.type=='108'|| data.type=='110' || data.type=='111' || data.type=='112'|| data.type=='113'){
					liRunData(data, conf);
					}else{
...
function liRunData(data, conf){
	...
	//getLiRunLv();
	var client=createMySQLClient();
		client.query("select value from ssc_params where name='LiRunLv'", function(err, rows, fields){
		if(err){
			log("读取系统彩盈利模式出错，随机开奖"+err);
			submitData(data, conf);
		}else{
			LiRunLv=rows[0]['value'];
			//console.log(LiRunLv);
	if(LiRunLv!=0){//盈利
	 var client=createMySQLClient();
	 client.query("select * from ssc_bets where type=? and actionNo=? and isDelete=0 and lotteryNo=''", [data.type, data.number], function(err, rows, fields){
		if(err){
			//log("读取投注出错联系技术qq272725553："+err);
			submitData(data, conf);
		}else{
			var a = 0;//奖金
			var b = 0;//本金
			for(var i=0;i<rows.length;i++){
				...
				try{
					...
					if(data.type==113){
						...
						var lhcWxHuo=setting['lhcWxHuo'];
						var lhcWxTu=setting['lhcWxTu']; 						
						alias=playedalias[rows[i]['playedId']];
						var zjCount=fun(rows[i]['actionData'], data.data, rows[i]['Groupname'],rows[i]['betInfo'], animalsYear, alias, lhcWxJin, lhcWxMu, lhcWxShui, lhcWxHuo, lhcWxTu)||0;						
					}else{
						var zjCount=fun(rows[i]['actionData'], data.data, rows[i]['Groupname'],rows[i]['betInfo'])||0;
					}
				
				zjAmount=rows[i]['odds']*zjCount*rows[i]['money'];//
				a+=rows[i]['odds']*zjCount*rows[i]['money'];//
			...
			if(a>b && qq272725553<LiRunLv){
				qq272725553++;
				log("盈利功能已打开，当前总购买金额"+b+"预计总中奖金额"+a+"系统判断当前期为亏损状态,自动提升难度等级为"+qq272725553);
				restartTask(conf, 0.1);
			}else if(b==0){
				log("盈利功能已打开，当前彩种没有投注，随机提交数据");
				submitData(data, conf);
			}else{
				log("盈利功能已打开，当前总购买金额"+b+"预计总中奖金额"+a+"系统判断当前期为盈利状态/或者已经达到难度上限，提交数据");
				qq272725553 = 0;
				submitData(data, conf);
			}
	 }});
