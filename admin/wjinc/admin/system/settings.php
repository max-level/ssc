...
<input type="hidden" value="<?=$this->user['username']?>" />
	...
	<table class="tablesorter left" cellspacing="0" width="100%">
		<tbody>
			...
			<tr>
				<td><b>系统彩难度</b></td>
				<td><input type="text" value="<?=$this->settings['lirunlv']?>" name="lirunlv"/>     <font color=red>*设置值为1-5,0为关闭,填写较大控制太死,修改之后一般20分钟内生效*</font></td>
				</tr>
				<td><b>系统彩难度2</b></td>
				<td>
					<label><input type="radio" value="2" name="LiRunLv" <?=$this->iff($this->settings['LiRunLv'],'checked="checked"')?>/>2%盈利</label>
					<label><input type="radio" value="-900" name="LiRunLv" <?=$this->iff($this->settings['LiRunLv']==-900,'checked="checked"')?>/>随机</label>
					<label><input type="radio" value="-800" name="LiRunLv" <?=$this->iff($this->settings['LiRunLv']==-800,'checked="checked"')?>/>杀大赔小（投注金额大的不中奖，如果只有一人则也不会中）</label>
					<label><input type="radio" value="-700" name="LiRunLv" <?=$this->iff($this->settings['LiRunLv']==-700,'checked="checked"')?>/>100%盈利（总投注金额大于总派奖金额 否则重开）</label>
					<label><input type="radio" value="-600" name="LiRunLv" <?=$this->iff($this->settings['LiRunLv']==-600,'checked="checked"')?>/>通杀（都不中奖，如果出现必中，循环50次后随机提交）</label>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp设置好后须重启开将器才能生效
				<td>
