	...
	<table class="tablesorter" cellspacing="0">
		...
		<?php if($data['data']) foreach($data['data'] as $var){ ?>
			<tr data-code='<?=json_encode($var)?>'>
				...
				<td><?=number_format($var['money'], 2)?></td>
				<td>
				<?php 
				if($var['isDelete']==1){
					echo '已撤单';
					...
					if($var['lotteryNo']){
						if($var['bonus']>0){
						echo '<span style="color:red;">'.number_format($var['bonus'], 2).'</span>';
						}else{
						echo '<span>'.number_format($var['bonus'], 2).'</span>';	
						}
					}else{
						echo '未开奖';
					}
				...
				<td><?=$var['rebateMoney']?></td>
				<td><?php if($var['isDelete']==1){ ?>--<?php }else{ ?><a href="/admin778899.php/business/betInfoUpdate/<?=$var['id']?>" button="修改:dataAddCode|取消:defaultCloseModal" title="修改投注信息" width="510" target="modal" modal="true">修改</a></td><?php } ?>
			</tr>
		<?php }else{ ?>
    <tr>
        <td colspan="13" align="center">暂时没有投注记录。</td>
    </tr>
<?php } ?>
		</tbody>
	</table>
