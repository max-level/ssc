...
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
  <tr>
    <td height="24" nowrap background="../images/06.gif"><font > <span class="STYLE2">KG投注记录记录</span></font></td>
  </tr>
  <tr>
...
$sql = "select id  from kgbetdetail WHERE 1=1 ";
...
// 	$sql   = "select * from ag_Transfer where id in($nid) ";
	$sql   = "select * from kgbetdetail where id in($nid) ";
	if($_GET['username'])$sql .="and username='".$_GET['username']."' ";
// 	if($_GET['live'] && $_GET['live']!="0")$sql .="and AG_type in(" . $_GET['live'] . ") ";
//     if($_GET['type'] && $_GET['type']!="0")$sql .="and AG_type in(" . $_GET['type'] . ") ";
	//if($_GET['live'] && $_GET['live']!="0")$sql .="and type in(" . $_GET['live'] . ") ";
...
?>
      <tr onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#FFFFFF'" style="background-color:#FFFFFF;">
        ...
        <td align="center" valign="middle"><?=date("Y-m-d H:i:s",strtotime($rows["CreateTime"]))?></td>
        <td align="center" valign="middle"><?=date("Y-m-d H:i:s",strtotime($rows["UpdateTime"]))?></td>
        <td align="center" ><?=$rows["GameCode"]?></td>
        <td align="center" ><?=$rows["BetType"]?></td>
        <td align="center" ><?=$rows["Amount"]?></td>
        <td align="center" ><?=$rows["Payout"]?></td>
        <td align="center" ><?=$rows["StakeAccurate"]?></td>
        ...
        <td align="center" ><?=$rows["BetSlip"]?></td>
       <!-- <td align="center"></td>-->
      </tr>
<?php
	}
