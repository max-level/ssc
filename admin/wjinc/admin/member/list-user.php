<?php
	...
	//判断在线的人数
	if($_GET['paixu'] == 'online')
	{
		$online_sql = "select * from ssc_member_session where isOnLine=1 AND ".time()." - accessTime < ".$GLOBALS['conf']['member']['sessionTime']." order by id desc";	//echo $online_sql;die;	
		$online = $this->getRows($online_sql);//在线的人
		//print_r($online);die;
		$sql = "select u.*,sum(b.money * b.totalNums) betAmount, sum(b.rebateMoney) rebateMoneyDouble, sum(b.bonus) zjAmount, (select sum(c.amount) from ssc_member_cash c where c.`uid`=u.`uid` and c.state=0 ) cashAmount,(select sum(r.amount) from ssc_member_recharge r where r.`uid`=u.`uid` and r.state in(1,2,9) ) rechargeAmount from ssc_members u left join ssc_bets b on u.uid=b.uid and b.lotteryNo !='' and b.isDelete=0  left join ssc_member_session ms on u.uid = ms.uid where u.admin=0";
		$uidArr = array();
		...
		//print_r($uidArr);
		$sql .= " AND u.uid IN (".join(',',$uidArr).") group by u.uid order by u.uid asc";
		//echo $sql;die;
	}
	...
	if($_GET['paixu'] == 'online')//如果是在线
	{
		...
		array_multisort($loginIPs, SORT_ASC, $data['data']);
	}
