...
class Biapi extends JoDES{
	...
	配置参数
	*/
	protected $key='XXXXXXXX';                                     //站点秘钥
	
	protected $WebSiteCode='a16291a1-e813-4d6e-a38b-6ebd105fe66d'; //站点code
	
	protected $password='aABb123Bb'; 								//站点用户密码
	...
	// 进入游戏
	public function loginbi($game,$user,$gametype=null,$devices=0)											//游戏登陆  游戏名  用户名
	{
		$t = time();
		if ($this->isMobile())
			$devices = 1;
		if($gametype!=null){
			$pp=($this->encode('platformCode='.$game.'&userName='.$user.'&userPassWord='.$this->password.'&gameType='.$gametype.'&TimeStamp='.$t,$this->key));
		}else{
			$pp=($this->encode('platformCode='.$game.'&userName='.$user.'&userPassWord='.$this->password.'&TimeStamp='.$t,$this->key));
		}
		$url='http://'.$this->biUrl.'/Api/Login?parameter='.$pp.'&WebSiteCode='.$this->WebSiteCode;
		return $url;
	}
	// 获取投注记录
