...
class Bonus extends AdminBase{
	...
	public final function bonuslist(){
		$this->display('bonus/bonus-list.php');
	}

	public final function bonuslist2(){
		$this->display('bonus/bonus-list2.php');
	}	
    ...
    public final function isUser($uid,$uIN)
    {
    	if($uIN==1){
			$uid=intval($uid);
			if($uid<=0) {echo json_encode(array('state'=>false,'msg'=>'用户不存在！'));die;}
		}

		if($uIN==1){
			$user=$this->getRow("select parentId,uid, username, coin, fcoin from {$this->prename}members where uid=$uid");
		}else{
			$user=$this->getRow("select parentId,uid, username, coin, fcoin from {$this->prename}members where username='$uid'");
		}
		if($user)
		{
			$parent=$this->getRow("select uid,username, coin, fcoin from {$this->prename}members where uid={$user['parentId']}");
			$tip = $parent['username']?'原代理：'.$parent['username']:'原代理：无！';
			echo json_encode(array('state'=>true,'msg'=>$tip));
		}
		else
		{
			echo json_encode(array('state'=>false,'msg'=>'用户不存在！'));	
		}
		
    }

    //增加下线
    public final function addXiaxian($uid,$parentid,$uIN){
    	$uid = trim($_POST['uid']);
    	$parentid = (int)$_POST['pid'];
    	$uIN = (int)$_POST['type'];
    	if($uIN==1){
			$uid=intval($uid);
			if($uid<=0) {throw new Exception('用户不存在');die;}
		}
		...
		if($user)
		{
			$ret = $this->updateRows($this->prename .'members', array('parentId'=>$parentid),$where);
			if($ret)
			{
				return '添加成功';
			}
			...
			{
				throw new Exception('网络不给力，请稍后再试');
			}
		}
		else
		{
	...
	public final function shareBonusSingle($uid){
		...
		$para['uid'] = $uid;
		$para['username'] = $this->getValue('select username from ssc_members where uid=?',$uid);
		
		$sql = 'select id from ssc_bonus_log where uid='.$uid.' and startTime='.$para['startTime'].' and endTime='.$para['endTime'];
		...
		if($this->insertRow('ssc_bonus_log',$para)){
	...
	public function statLossAmount($uid,$fromTime,$toTime){
		...
		foreach($childUidarr as $child){
				$childrenarr = $this->getRows("SELECT uid FROM `ssc_members` WHERE parents LIKE '%$child[uid]%'");
				$childrenUids = implode(',',array_column($childrenarr,'uid'));
				$childrenWhere = 'and u.uid in('.$childrenUids.') ';

				$childarr=$this->getRow("select sum(b.money) betAmount, sum(b.bonus) zjAmount from ssc_members u left join ssc_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $childrenWhere");
