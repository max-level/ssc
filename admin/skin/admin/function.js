...
function userDataBeforeSubmitCode(){
	...
	if(this.coin.value<0) throw('设置有问题');
	if(!this.fanDian.value.match(/^[\d\.\%]{1,4}$/)) throw('请正确设置返点');
...
// 弹出彩金赠送页
function rechargModalget(){
	$.get('/admin778899.php/' + 'business/rechargeModalget', function(html){
			...
			buttons:{
				"确定充值":function(event, ui){
					...
					try{
						...
						if(!amount.match(/^-?\d+(\.\d{0,2})?$/)) throw('赠送金额错误');
						amount=parseFloat(amount).toFixed(2);
						//if(!isNaN(min) && min>amount) throw('充值金额不能少于'+min);
					}catch(err){
						error(err);
					...
					$.ajax( '/admin778899.php/' + 'business/rechargegetAction/' + uid + '/' + amount+ '/' + userRid, {
						dataType:'json',
						error:defaultError,
						success:defaultSuccess
					});
 ...
 */
function addCoinLog(){
	$.get('/admin778899.php/business/addCoinLogModal', function(html){

		$(html).dialog({
			title:'添加帐变记录',
			width:440,
			buttons:{
				"添加":function(event, ui){
					var $this=$(this),
					$username=$('input[name=username]', this),
					$amount=$('input[name=amount]', this),
					$liqType=$('select[name=liqType]', this),
					$info=$('input[name=info]', this),
					$extfield0=$('input[name=extfield0]', this),
					$extfield1=$('input[name=extfield1]', this),
					$extfield2=$('input[name=extfield2]', this),
					...
					liqType=$liqType.val(),
					info=$info.val(),
					extfield0=$extfield0.val(),
					extfield1=$extfield1.val(),
					extfield2=$extfield2.val();
					
					try{
						if(username=="")  throw('用户名不能为空');
						if(!amount.match(/^-?\d+(\.\d{0,2})?$/)) throw('充值金额错误');
						amount=parseFloat(amount).toFixed(2);
						if(liqType=="")  throw('类型不能为空');
						if(isNaN(liqType)) throw('类型必须是数字');
						if(info=="")  throw('info不能为空');
					}catch(err){
						error(err);
						return;
					}
					$(this).dialog("destroy");
					$.ajax('/admin778899.php/business/addCoinLogSubmit', {
						data:{
							username:username,
							amount:amount,
							liqType:liqType,
...
function clearSettings(err, data){
	if(err){
		error(err);
	}else{
		success('数据清理设置成功');
...
function conCommHandle(err, data){
	if(err){
		...
		if(data) success('已成功更新所有会员发放状态为未领取');
...
function sysReloadNoticeposs(err, data){
	if(err){
		error(err);
	}else{
		success(data);
		load('manage/permission');
	}
}
function sysReloadNotice1(err, data){
	if(err){
		error(err);
	}else{
		success(data);		
...
function sysReloadNotice2(err, data){
	if(err){
		error(err);
	}else{
		success(data);
		load('system/stop_bankcard');		
	}
}

function sysReloadNotice3(err, data){
	if(err){
		error(err);
	}else{
		success(data);
...
function sysReloadNotice4(err, data){
	if(err){
		error(err);
	}else{
		success(data);
...
function sysEditwithdraw(id){

	$('<div class="bank-modal"><iframe frameborder="0" src="/admin778899.php/system/withdrawModal/'+(id||'')+'" style="width:480px;height:230px;"></iframe></div>')
	.dialog({
		title:'编辑提款类型',
		width:500,
		buttons:{
...
function onUpdatewithdraw(type, msg){
	var err, data;
	$('.bank-modal').dialog('destroy').remove();
	if(type=='error'){
		err=msg;
	...
	sysReloadwithdraw(err, data);
}

function sysReloadwithdraw(err, data){
	if(err){
		error(err);
	}else{
		if(data) success(data);
		load('system/withdraw');
	}
}
...
function clearDataBefor3(){
	var date=$("#clearData3").val();
	if(!date) throw('日期未填');
	$(this).data('date', date);
}
function clearDataSuccess3(err, msg){
	if(err){
		alert(err);
	}else{
		alert("成功清除！");;
...
function clearDataBefor4(){
	var date=$("#clearData4").val();
	if(!date) throw('日期未填');
	$(this).data('date', date);
}
function clearDataSuccess4(err, msg){
	if(err){
		alert(err);
	}else{
		alert("成功清除！");;
...
function clearDataBefor5(){
	var date=$("#clearData5").val();
	if(!date) throw('日期未填');
	$(this).data('date', date);
}
function clearDataSuccess5(err, msg){
	if(err){
		alert(err);
	}else{
		alert("成功清除！");;
...
function clearDataBefor6(){
	var date=$("#clearData6").val();
	if(!date) throw('日期未填');
	$(this).data('date', date);
}
function clearDataSuccess6(err, msg){
	if(err){
		alert(err);
	}else{
		alert("成功清除！");;
...
function clearDataBefor7(){
	var date=$("#clearData7").val();
	if(!date) throw('日期未填');
	$(this).data('date', date);
}
function clearDataSuccess7(err, msg){
	if(err){
		alert(err);
	}else{
		alert("成功清除！");;
...
function beforeSysSettings(){
	document.getElementById("modeY").value = document.getElementById("modeY").checked ? "1":"0";
	document.getElementById("modeJ").value = document.getElementById("modeJ").checked ? "1":"0";
	document.getElementById("modeF").value = document.getElementById("modeF").checked ? "1":"0";
	document.getElementById("modeL").value = document.getElementById("modeL").checked ? "1":"0";
}

function beforeEditSave(){
	var w=document.getElementsByName("cb[]");
...
function beforeDealRecharge(){
	//document.getElementById("gifted").value = document.getElementById("gifted").checked ? "1":"0";
	$('form', this).trigger('submit');
}

function sysModal(modalPath, titl, width){
	$.get('/admin778899.php/' + modalPath, function(html){
		$(html).dialog({
			title: titl,
			width: width,
			buttons:{
...
//修改提交后的通用回调函数（用于刷新列表上的操作）
function sysReload(err, data){ if(err){	error(err);}else{ if(data) success(data); reload();}}

function modalCallback(err, data){
	if(err){
		alert(err);
	}else{
		$(this).parent().dialog('destroy');
