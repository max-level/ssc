<?php
	...
	if($_REQUEST['fromTime'] && $_REQUEST['toTime']){
		...
		$toTime=strtotime('00:00')+24*3600;
		$timeWhere=' and l.actionTime<'.$toTime;
	}
	if($this->user['uid']==1){
	   $sql="select l.*, u.username from {$this->prename}admin_log l, {$this->prename}sysmember u where l.uid=u.uid $timeWhere $typeWhere $userWhere order by l.id desc";
	}else{
	   $sql="select l.*, u.username from {$this->prename}admin_log l, {$this->prename}sysmember u where l.uid=u.uid $timeWhere $typeWhere $userWhere and u.uid={$this->user['uid']} order by l.id desc";
	}
