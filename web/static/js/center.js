...
var User = {
	...
	updatePwd: function() {
		...
		if(!loginpwd) {layer.alert('请输入当前登录密码', {icon : 2});return false;}
...
var Pay = {
	...
	initPayType : function() {
		...
		$(".tab").click(function() {
			var a = $(this).attr("id");
			$(".subcontent").hide();
			$(".tab").removeClass("tabnaviactive");
			$("#subpage" + a).show();
			$(this).addClass("tabnaviactive");
		...
		$.ajax({
			...
			success : function(data) {
				...
				var tabs = $('.mt10 > .tab:not(".hide")');
				if (tabs.length == 0) {
					return false;
				}
				var firstTab = tabs[0];
				$(firstTab).addClass("tabnaviactive");
...
//		}
	...
	getSelectTabId : function() {
		return $('.mt10 > .tabnaviactive').attr("id");
	},
...
var WithDraw = {
	init : function() {
		if (Center.user.superName == "") {
			$('#withdrawal0').show();
		}
	
		// 判断用户余额是否达到提款最小限额，否则提示用户先充值
		else if (Center.user.money < 100) {
			$('#withdrawal1').show();
		}
		else if (Center.user.superName == null) {
			$('#withdrawal0').show();
		}
		// 未绑定银行卡，先让用户绑定银行卡
				...
				if (result && result.totalCount > 0) {
					var bodyHtmlStrFormat = '\
			    		 <tr class="borderbtmgrey">\
							...
							<td>{uwType}</td>\
							<td>-{applyMoney}</td>\
							<td>提取-{applyMoney} 元 到 [{reason}]{reason1} {reason2}</td>\
							<td>{checkStatus}</td>\
						</tr>\
			    		 ';
