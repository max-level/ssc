var UserBetBill ={
	...
	changeGame:function(){
		 var date=HttpUtil.GetUrlPara()["date"].split(" ")[0];
...
//            maxmin: true, //开启最大化最小化按钮
            area: ['950px', '590px'],
            content: '/game/include/bet_list.html?t='+ Date.parse(new Date())+"&settled="+settled
        });
	},
	getBetBills:function(pageOption){
		var settled=HttpUtil.GetUrlPara()["settled"];
		$("#column_desc").html(settled == "true" ? "结果" : "可赢金额");
		var requestUrl="/report/getBetBills.do";//今日注单
		if(pageOption.date){
			requestUrl="/report/getUserBets.do";//历史注单
		}
		...
		$.ajax({
			  ...
			  success: function(result){
			     ...
			     if(result && result.totalCount>0){
			    	 ...
			    	 $("#bet_list_body").html('<tr><td colspan="7">暂无数据</td></tr>');
			     }
			   }
			});
	}
