...
class Team extends WebLoginBase{
	...
	public final function insertMember(){
		...
		if($update['type']!=0 && $update['type']!=1  && $update['type']!=2 && $update['type']!=3) throw new Exception('类型出错，请重新操作');
 ...
 /*额度转移*/
	public final function userUpdateed2(){
		...
		if($userData2['zparentId']!=$uid2 || $userData2['parentId']) throw new Exception('此会员不是的你的直属会员，请重新选择！');
		}elseif($this->user['type']==3){ 		
		if($userData2['gudongId']!=$uid2 || $userData2['zparentId'] || $userData2['parentId']) throw new Exception('此会员不是的你的直属会员，请重新选择！');
		}
