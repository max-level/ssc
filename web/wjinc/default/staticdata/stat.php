...
if($gameId==1 || $gameId==73 || $gameId==80 || $gameId==81 || $gameId==82 || $gameId==100 || $gameId==101 || $gameId==102 || $gameId==103 || $gameId==122 || $gameId==119 || $gameId==120 || $gameId==121){
	$sql=" and (name='大' or name='小' or name='单' or name='双' or name like '总和%' or name like '尾%' or name like '合数%' )";	
}elseif($gameId==50 || $gameId==51 ||$gameId==52 ||$gameId==58 ||$gameId==59 || $gameId==55 || $gameId==72 || $gameId==108 || $gameId==104){
...
$clong=$this->getRows("select id, name, played_groupid from {$this->prename}played where type={$gameId} {$sql} order by id desc");//倒序
//var_dump($clong);
$curStatList=array();
$statarr=array();
if($clong){
